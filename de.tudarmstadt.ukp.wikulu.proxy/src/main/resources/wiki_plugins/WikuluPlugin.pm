...
package TWiki::Plugins::WikuluPlugin;
...
# This should always be $Rev: 15942 (06 Dec 2008) $ so that TWiki can determine the checked-in
# status of the plugin. It is used by the build automation tools, so
# you should leave it alone.
$VERSION = '$Rev: 15942 (06 Dec 2008) $';
...
$SHORTDESCRIPTION = 'WikuluPlugin provides an API for the Wikulu project';
...
sub initPlugin {
    ...
    TWiki::Func::registerRESTHandler('wikicontent', \&restWikiContent);
    TWiki::Func::registerRESTHandler('writewikicontent', \&restWriteWikiContent);
    TWiki::Func::registerRESTHandler('alltopics', \&restAllTopics);
    TWiki::Func::registerRESTHandler('alltopicsinweb', \&restTopicsInWeb);
...
sub restWriteWikiContent {
    my ($web, $topic) = TWiki::Func::normalizeWebTopicName(
        undef, TWiki::Func::getCgiQuery()->param('topic'));
    my ($meta, $text) = TWiki::Func::readTopic($web, $topic);
    my $new_text = TWiki::Func::getCgiQuery()->param('text');
    TWiki::Func::saveTopic( $web, $topic, $meta, $new_text, undef);
    return "info: writing to wiki successful";
}
...
sub restAllTopics {	
	...
	foreach (@webs) {
		my $web = $_;
		my @webTopics = TWiki::Func::getTopicList($web);
		
		foreach (@webTopics) {
			my $webTopic = $web . '/' . $_;			
			push(@allTopics,$webTopic)	
		}	
	}
