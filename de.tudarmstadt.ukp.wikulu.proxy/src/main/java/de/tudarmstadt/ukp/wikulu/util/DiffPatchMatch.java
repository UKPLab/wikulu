...
public class DiffPatchMatch
{
	...
	public float Match_Balance = 0.3f;
	...
	public DiffPatchMatch()
	{
		// load properties to overwrite matching settings
		ResourceBundle properties = PropertyResourceBundle.getBundle("wikulu");
		float balance = Float.parseFloat(properties
				.getString("wikulu.match.balance"));
		float threshold = Float.parseFloat(properties
				.getString("wikulu.match.threshold"));
		...
		for (int d = 0; d < max_d; d++) {
			...
			if (doubleEnd) {
				...
				for (int k = -d; k <= d; k += 2) {
					...
					y = x - k;
					footstep = diff_footprint(text1.length() - x,
							text2.length() - y);
					if (!front && (footsteps.containsKey(footstep))) {
						done = true;
					}
					...
					while (!done
							&& x < text1.length()
							&& y < text2.length()
							&& text1.charAt(text1.length() - x - 1) == text2
									.charAt(text2.length() - y - 1)) {
						x++;
						y++;
						footstep = diff_footprint(text1.length() - x,
								text2.length() - y);
					...
					if (done) {
						...
						LinkedList<Diff> a = diff_path1(v_map1,
								text1.substring(0, text1.length() - x),
	...
	public int diff_commonSuffix(String text1, String text2)
	{
		...
		int n = Math.min(text1.length(), text2.length());
		for (int i = 0; i < n; i++) {
			if (text1.charAt(text1.length() - i - 1) != text2.charAt(text2
					.length() - i - 1)) {
				return i;
			}
		}
	...
	protected int match_bitap(String text, String pattern, int loc)
	{
		...
		if (best_loc != -1) {
			score_threshold = Math.min(
					match_bitapScore(0, best_loc, loc, score_text_length,
		...
		for (int d = 0; d < pattern.length(); d++) {
			...
			while (bin_min < bin_mid) {
				if (match_bitapScore(d, bin_mid, loc, score_text_length,
						pattern) < score_threshold) {
					bin_min = bin_mid;
				}
				else {
			...
			int finish = Math
					.min(text.length() - 1, pattern.length() + bin_mid);
			...
			for (int j = finish - 1; j >= start; j--) {
				if (d == 0) {
					...
					rd[j] = ((rd[j + 1] << 1) | 1)
							& (s.containsKey(text.charAt(j)) ? s.get(text
									.charAt(j)) : 0);
				}
				else {
				...
				if ((rd[j] & matchmask) != 0) {
					double score = match_bitapScore(d, j, loc,
							score_text_length, pattern);
