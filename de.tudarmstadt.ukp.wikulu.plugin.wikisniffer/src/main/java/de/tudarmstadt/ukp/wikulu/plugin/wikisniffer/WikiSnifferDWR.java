 ...
 *******************************************************************************/
package de.tudarmstadt.ukp.wikulu.plugin.wikisniffer;
...
public class WikiSnifferDWR {
	...
	/**
	 * determines whether to return a "real" summary
	 * or just the first paragraph of an article
	 * 
	 */
	private boolean realSummary;
	/**
	 * a list of headings
	 */
...
//}
	...
	private void extractSummary() {
		...
		this.summary = this.wikiController.getArticleText();
		...
		if(this.realSummary) {
			...
			if(this.summary.length() > 0) {
				try {
					String jsonString = "{ \"text\" : \"" + URLEncoder.encode(this.summary, "UTF-8") + "\" }";
					summaryString = sp.run(jsonString);
					JSONObject summaryJson = new JSONObject(summaryString);
					JSONArray sumArray = summaryJson.getJSONArray("summary").getJSONArray(0);
					//System.out.println(sumArray.toString(2));
					StringBuilder cleanText = new StringBuilder();
					for(int x=0; x<sumArray.length(); x++) {
						cleanText.append(sumArray.getString(x) + "<br />");
					} 
					//summary = summaryJson.getString("summary");
					summary = cleanText.toString();
				} catch (Exception e) {
					e.printStackTrace();
					summary = "";
				} 
			...
			// return the first paragraph as summary
			// TODO: this works for MediaWiki only! for
			// TWiki, the flag realSummary in wikulu.properties
			// has to be true!
			this.summary = this.summary.split("<!-- paragraph -->")[0];
	...
	private void extractExtraSummary() {
		...
		System.out.println("EXTRA SUMMARY: " + summary);
