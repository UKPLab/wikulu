...
function hoverLink(hrefNode, sniffer) {
	...
	if (visited.html() == null) {
		...
		sniffer.initialize(hrefNode, mPosX, mPosY, uid);
		sniffer.execute();
	} else {
...
function Sniffer(parentSnifferSdNodes) {
	...
	this.initialize = function(hrefNode, mPosXVar, mPosYVar, uid) {
		this.mPosX = mPosXVar;
		this.mPosY = mPosYVar;
		this.uid = uid;
	...
	this.executeAppendToWindow = function() {
		...
		var targetLink ;
		var linkTo = this.hrefNode.attr("href").split(",",2)[0];
		if(this.hrefNode.attr("href").indexOf("http") ==0){
			targetLink = this.hrefNode.attr("href");
		} else{
			if(port == ""){
				targetLink = new String(location.protocol+"//"+document.domain+linkTo);
			}else{
				targetLink = new String(location.protocol+"//"+document.domain+":"+port+linkTo);
			}
		}
		...
		WikiSniffer.getWikiSniffer(targetLink, viewURL, apiURL, wikiSyntax,
				   ...
				   enableExternalLinks, enableBacklinks, enableImages, realSummary, 
	...
	this.execute = function() {
		...
		var port = location.port;
		// some links are broken and contain commas, so
		// use just the first "real" href
		var linkTo = this.hrefNode.attr("href").split(",",2)[0];
		var targetLink;
	...
	this.setKeyphrases = function(val, node) {
		...
		if (sortedKx.length > 0) {
			for (var i = 0; i < sortedKx.length;) {
				...
				content += "<span style=\"font-size:"+sortedKx[i][1]+"%\"><a href=\""+escape(sortedKx[i][0])+"\" title=\"Click to search for &quot;"+keyphrase+"&quot;\" class=\"ws_keyphrase\">"+keyphrase+"<\/a><\/span>";
